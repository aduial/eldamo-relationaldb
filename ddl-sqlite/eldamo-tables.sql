...
CREATE TABLE DOC (
	...
	CONSTRAINT FK_DOC_TYPE FOREIGN KEY (DOCTYPE_ID) REFERENCES "TYPE"(ID)
);
...
CREATE INDEX RELATION_TO_ID_IDX ON RELATION (TO_ID);
...
CREATE TABLE ENTRY (
    ...
    ENTRY_TYPE_ID integer NOT NULL, REF_SOURCES VARCHAR(255),
    PRIMARY KEY (ID),
...
CREATE INDEX ENTRY_PARENT_ID_2_IDX ON ENTRY (PARENT_ID);
CREATE INDEX ENTRY_PARENTID_ID_IDX ON ENTRY (PARENT_ID,ID);
CREATE INDEX ENTRY_FORMID_IDX ON ENTRY (FORM_ID);
CREATE INDEX ENTRY_SOURCE_IDX ON ENTRY ("SOURCE");
CREATE INDEX ENTRY_FORMID_SOURCE_IDX ON ENTRY (FORM_ID,"SOURCE");
CREATE INDEX ENTRY_FORM_LANG_IDX ON ENTRY (FORM_ID,LANGUAGE_ID);