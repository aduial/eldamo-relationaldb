...
sub hashlangs {
    ...
    foreach my $language ( sort { $a <=> $b } keys %languages ) {
        ...
        push @lang_rows, "($language, '$languages{$language}{name}', '$languages{$language}{lang}', $languages{$language}{parent}, $languages{$language}{order}, $languages{$language}{ctg})"; # added 999 for 
...
sub hashglosses {
    ...
    foreach my $gloss ( sort( unique(@raw_glosses) ) ) {
        if ( $gloss ne '' ) {
            ...
            push @gloss_rows, "($gloss_uid, 100, '$gloss')";
...
sub loadvariables {
    ...
    $insert_language =
      'INSERT INTO ' . $schema . 'LANGUAGE (ID, NAME, LANG, PARENT_ID, LIST_ORDER, CATEGORY) VALUES ';
    $insert_source =
        'INSERT INTO '
      . $schema
    ...
    %languages = (
        ...
        7 => {
            parent => 1,
            lang   => "pn",
            name   => "Primitive Elvish (incl neo-)",
            order  => 70,
            ctg    => 2,
        },
        8 => {
